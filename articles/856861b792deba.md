...
つまり、`getUsers()`関数の呼び出し元では下記のように`await getUsers()`のように書くことになり、`getUsers()`内部で`await res.json()`と書いても書かなくても、動作に違いはありません（返されるのはいずれも `Promise<User[]>` であり、呼び出し元での扱いが変わらないためです）。
...
また、TypeScriptの世界で考えた場合、`getUsers()`の戻り値の型に着目すると、**`await res.json()`と書いても`res.json()`と書いても型はどちらも`Promise<User[]>`**になります。
...
このようにシグネチャの観点から見ても、内部で`res.json()`に`await`を使用しているかどうかに関わらず、呼び出し元でその実行結果を参照する場合は、`await`を使用することで`Promise<User[]>`から`User[]`を取得する必要があることがわかります。
...
小さな改善の積み重ねが、コードの読みやすさとパフォーマンスの向上につながります。